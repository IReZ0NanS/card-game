...
const DEFAULT_CARDS_DB = [
  {
    ...
    name: "Учень Академії",
    rarity: "Звичайна",
    image: "https://placehold.co/400x600/222/aaa?text=Учень\nАкадемії",
    maxSupply: 0,
    pulledCount: 0,
    sellPrice: 15,
  },
  {
    id: "c2",
    packId: "p1",
    name: "Тренувальний манекен",
    rarity: "Звичайна",
    image: "https://placehold.co/400x600/222/aaa?text=Манекен",
    maxSupply: 0,
    pulledCount: 0,
    sellPrice: 15,
  },
  {
    ...
    name: "Генін",
    rarity: "Рідкісна",
    image: "https://placehold.co/400x600/1e3a8a/ccc?text=Генін",
    maxSupply: 0,
    pulledCount: 0,
    sellPrice: 30,
  },
  {
    ...
    name: "Елітний Джонін",
    rarity: "Епічна",
    image: "https://placehold.co/400x600/581c87/eee?text=Елітний\nДжонін",
    maxSupply: 500,
    pulledCount: 0,
    sellPrice: 100,
  },
  {
    id: "c8",
    packId: "p2",
    name: "Легендарний Хокаге",
    rarity: "Легендарна",
    image: "https://placehold.co/400x600/854d0e/fff?text=Легендарний\nХокаге",
    maxSupply: 10,
    pulledCount: 0,
    sellPrice: 500,
  },
...
const COLOR_PRESETS = {
  ...
  yellow: {
    border: "border-yellow-400 shadow-yellow-500/80",
...
const getCardStyle = (rName, raritiesList) => {
  ...
  return r && COLOR_PRESETS[r.color]
    ? COLOR_PRESETS[r.color]
    : COLOR_PRESETS["gray"];
};
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (loading && user !== undefined && !profile && !needsRegistration) {
      timeout = setTimeout(() => {
        setDbError(
          "Зв'язок з сервером втрачено. Перевірте підключення до інтернету або налаштування Firebase."
        );
        setLoading(false);
      }, 8000);
    }
  ...
  const handleAuthSubmit = async (e) => {
    ...
    try {
      if (authMode === "register") {
        ...
        await setDoc(
          doc(db, "artifacts", GAME_ID, "public", "data", "profiles", newUid),
          {
            uid: newUid,
            nickname,
            email,
      ...
      if (err.code === "auth/invalid-credential")
        setDbError("Невірна пошта або пароль.");
      else if (err.code === "auth/email-already-in-use")
        setDbError("Акаунт з такою поштою вже існує.");
      else if (err.code === "auth/weak-password")
        setDbError("Пароль надто короткий (мінімум 6 символів).");
      else if (err.code === "auth/invalid-email")
        setDbError("Неправильний формат пошти.");
  ...
  const openPack = async (packId, cost, amountToOpen = 1) => {
    ...
    setTimeout(
      async () => {
        ...
        let results = [];
        let countsMap = {};
        const availablePackCards = tempCatalog.filter(
          (c) => c.packId === packId
        );
        ...
        for (let i = 0; i < amountToOpen; i++) {
          const availableNow = tempCatalog.filter(
            (c) =>
              c.packId === packId &&
              (!c.maxSupply || (c.pulledCount || 0) < c.maxSupply)
          );
          ...
          for (const c of availableNow) {
            ...
            if (c.weight !== undefined && c.weight !== "") {
              ...
              selectedPackDef?.customWeights?.[c.rarity] !== undefined &&
              selectedPackDef?.customWeights?.[c.rarity] !== ""
            ) {
        ...
        try {
          ...
          if (amountToOpen === 1) {
            const fakeCards = Array.from(
              ...
              () =>
                availablePackCards[
                  Math.floor(Math.random() * availablePackCards.length)
                ]
            );
  ...
  const sellDuplicate = async (cardId) => {
    ...
    const cardPrice = cardDef?.sellPrice
      ? Number(cardDef.sellPrice)
      : SELL_PRICE;
  ...
  // МАСОВИЙ ПРОДАЖ УСІХ ДУБЛІКАТІВ У ІНВЕНТАРІ
  const sellEveryDuplicate = async () => {
    ...
    try {
      await batch.commit();
      showToast(
        `Продано всі дублікати! Отримано ${totalEarned} монет.`,
        "success"
      );
    } catch (e) {
      console.error(e);
    ...
    return (
      <div className="min-h-screen bg-neutral-950 flex flex-col items-center justify-center p-6 text-center animate-in fade-in zoom-in duration-500">
        <div className="bg-red-950/40 border-2 border-red-900 p-8 rounded-3xl max-w-lg w-full shadow-[0_0_50px_rgba(220,38,38,0.3)] relative overflow-hidden">
          ...
          />
          <h1 className="text-2xl font-black mb-4 text-white uppercase tracking-widest drop-shadow-md">
            Увага!
          </h1>
  ...
  if (profile?.isBanned) {
    ...
    const banDateStr = isTempBan
      ? new Date(profile.banUntil).toLocaleString("uk-UA")
      : "";
    ...
    return (
      <div className="min-h-screen bg-neutral-950 flex flex-col items-center justify-center p-6 text-center animate-in fade-in zoom-in duration-500 relative overflow-hidden">
        ...
        <div className="bg-red-950/30 border-2 border-red-900/50 p-10 rounded-3xl max-w-md w-full relative z-10 shadow-[0_0_80px_rgba(220,38,38,0.15)]">
          <Ban
            size={80}
            className="mx-auto mb-6 text-red-500 drop-shadow-[0_0_15px_rgba(220,38,38,0.8)]"
          />
          <h1 className="text-4xl font-black mb-2 text-white tracking-wider">
            ВИ ЗАБАНЕНІ
          </h1>
          <p className="text-red-400 font-bold uppercase tracking-widest mb-8 text-sm">
            Доступ до гри обмежено
          </p>
          ...
          <div className="bg-neutral-950 border border-neutral-800 rounded-xl p-5 mb-8 text-left">
            <div className="mb-4">
              ...
              <div className="text-white text-lg">
                {profile.banReason || "Порушення правил гри."}
              </div>
            </div>
            <div>
              <div className="text-xs text-neutral-500 uppercase font-bold mb-1">
              ...
              <div className="text-red-400 font-bold text-lg">
                {isTempBan ? `До ${banDateStr}` : "Назавжди (Перманентно)"}
              </div>
            </div>
          </div>
          ...
          <button
            ...
            className="w-full bg-neutral-900 hover:bg-neutral-800 text-white border border-neutral-700 font-bold py-4 rounded-xl transition-colors flex justify-center items-center gap-2"
          >
  ...
  if (loading || user === undefined) {
    return (
      <div className="min-h-screen bg-neutral-950 flex flex-col items-center justify-center text-yellow-500">
        ...
        <p className="text-neutral-500 font-bold uppercase tracking-widest animate-pulse">
          З'єднання...
        </p>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-neutral-100 font-sans pb-24">
            ...
            <div
              className={`w-10 h-10 rounded-full flex items-center justify-center font-bold border shadow-sm ${
                profile?.isSuperAdmin
                  ...
                  : "bg-neutral-800 border-neutral-700 text-yellow-500"
              }`}
            >
              {profile?.isSuperAdmin ? (
                <Crown size={20} />
      ...
      {toastMsg.text && (
        <div
          className={`fixed top-20 left-1/2 -translate-x-1/2 px-6 py-3 rounded-full flex items-center gap-2 shadow-lg z-50 animate-bounce text-white font-medium ${
      ...
      <nav className="fixed bottom-0 w-full bg-neutral-900 border-t border-neutral-800 px-4 py-2 z-40 shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
        <div className="max-w-md mx-auto flex justify-around relative">
          ...
          <NavButton
            ...
            label="Рейтинг"
            isActive={
              currentView === "rating" || currentView === "publicProfile"
            }
  ...
  return (
    <button
      ...
      className={`flex flex-col items-center p-2 rounded-lg w-16 sm:w-20 transition-colors ${
        isActive ? "text-yellow-500" : "text-neutral-500 hover:text-neutral-300"
      }`}
    >
      {icon}
        ...
        <div className="relative w-full max-w-4xl mx-auto bg-neutral-900 border-[4px] border-neutral-700 rounded-3xl h-72 shadow-[0_0_50px_rgba(0,0,0,0.8)] overflow-hidden">
          ...
          <div
            ...
            style={{
              ...
              transition:
                roulettePos === 1
                  ? "transform 4.5s cubic-bezier(0.1, 0.85, 0.1, 1)"
                  : "none",
              transform:
                roulettePos === 1
          ...
          >
            {rouletteItems.map((item, i) => {
              ...
              return (
                <div
                  key={i}
                  className={`w-40 h-56 rounded-2xl border-4 shrink-0 bg-neutral-950 relative overflow-hidden shadow-xl ${style.border}`}
                >
                  <img
                    src={item.image}
                    alt="card"
    ...
    return (
      <div className="flex flex-col items-center min-h-[65vh] animate-in zoom-in-95 duration-700 w-full pb-10">
        <h2 className="text-3xl sm:text-4xl font-black mb-8 text-white uppercase tracking-widest text-center drop-shadow-[0_0_15px_rgba(255,255,255,0.5)]">
          Ви отримали{" "}
          {pulledCards.length > 1 ? `(${pulledCards.length} шт)` : "!"}
        </h2>
        ...
        <div className="flex flex-wrap justify-center gap-6 mb-10 w-full">
          {pulledCards.map((card, index) => {
            ...
            return (
              <div
                key={index}
                className="flex flex-col items-center animate-in zoom-in slide-in-from-bottom-6"
                style={{ animationDelay: `${index * 150}ms`, fillMode: "both" }}
              >
                <div
                  className={`w-40 sm:w-56 aspect-[2/3] rounded-2xl border-4 overflow-hidden shadow-[0_0_40px_rgba(0,0,0,0.6)] transform transition-all hover:scale-105 hover:rotate-2 ${style.border} bg-neutral-900 relative mb-4`}
                >
                  <img
                    src={card.image}
                    alt={card.name}
                ...
                <div className="text-center">
                  <div
                    className={`text-xs font-black uppercase tracking-widest flex justify-center items-center gap-1 ${style.text}`}
                  >
                    <Sparkles size={14} /> {card.rarity}
                  </div>
                  <h3 className="font-bold text-white max-w-[150px] truncate">
                    {card.name}
                  </h3>
                </div>
              </div>
        ...
        <div className="flex flex-col items-center mb-12 bg-neutral-900/50 p-6 rounded-3xl border border-neutral-800 max-w-2xl mx-auto">
          ...
          <div className="flex flex-wrap justify-center gap-3 w-full">
            <OpenButton
              ...
              opening={openingPackId === selectedPack.id}
            />
            <OpenButton
              amount={5}
              cost={selectedPack.cost}
              ...
              opening={openingPackId === selectedPack.id}
              color="bg-orange-500 hover:bg-orange-400 text-orange-950"
            />
            <OpenButton
              amount={10}
              cost={selectedPack.cost}
              ...
              opening={openingPackId === selectedPack.id}
              color="bg-red-500 hover:bg-red-400 text-red-950"
            />
          </div>
        </div>
        ...
        <div className="border-t border-neutral-800 pt-8 w-full">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-4">
            {packCards.map((card) => {
              const style = getCardStyle(card.rarity, rarities);
              const isSoldOut =
                card.maxSupply > 0 && (card.pulledCount || 0) >= card.maxSupply;
              return (
                <div
                  key={card.id}
                ...
                >
                  <div
                    className={`relative w-full aspect-[2/3] rounded-xl border-2 overflow-hidden bg-neutral-900 mb-2 transition-all duration-300 ${
                      !isSoldOut &&
                      "group-hover:-translate-y-2 group-hover:shadow-[0_10px_20px_rgba(0,0,0,0.5)]"
                    } ${style.border}`}
                  >
                    <img
                      src={card.image}
                    ...
                    {card.maxSupply > 0 && (
                      <div className="absolute top-1 right-1 bg-black/90 text-white text-[8px] px-1.5 py-0.5 rounded border border-neutral-700 font-bold z-10">
                        {isSoldOut
                          ? "РОЗПРОДАНО"
                          : `${card.maxSupply - (card.pulledCount || 0)}/${
                              card.maxSupply
                            }`}
                      </div>
                    )}
    ...
    <div className="pb-10 animate-in fade-in zoom-in-95 duration-500">
      <div className="text-center mb-8">
        ...
        <p className="text-neutral-400 text-sm">
          Оберіть пак, Мій лорд, і випробуйте удачу!
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
        {packs.map((pack) => (
          <button
            ...
            className="bg-neutral-900 border border-neutral-800 rounded-3xl p-6 flex flex-col items-center justify-between group hover:border-neutral-600 transition-colors shadow-lg text-left w-full cursor-pointer hover:-translate-y-1 transform duration-300"
          >
...
function OpenButton({
  ...
  onClick,
  opening,
  color = "bg-yellow-500 hover:bg-yellow-400 text-yellow-950",
}) {
...
function InventoryView({
  ...
  const sortedInventory = [...inventory].sort((a, b) => {
    const getWeight = (rName) =>
      rarities.find((x) => x.name === rName)?.weight || 100;
    if (sortBy === "rarity")
  ...
  const duplicatesEarnedCoins = inventory.reduce((sum, item) => {
    if (item.amount > 1) {
      const cardPrice = item.card.sellPrice
    ...
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4 bg-neutral-900/80 p-5 rounded-2xl border border-neutral-800 shadow-lg">
        <h2 className="text-2xl font-black flex items-center gap-3 text-white uppercase tracking-wider">
          <LayoutGrid className="text-yellow-500 w-8 h-8" /> Моя Колекція{" "}
          <span className="text-neutral-500 text-lg">
            ({inventory.length}/{catalogTotal})
          </span>
        </h2>
          ...
          {duplicatesEarnedCoins > 0 && (
            <button
              onClick={() => {
                if (
                  confirm(
                    `Продати всі дублікати та отримати ${duplicatesEarnedCoins} монет?`
                  )
                ) {
                  sellEveryDuplicate();
                }
            ...
            >
              <Zap size={18} /> Продати дублікати (+{duplicatesEarnedCoins}{" "}
              <Coins size={14} />)
            </button>
          )}
            ...
            return (
              ...
              >
                ...
                <div className="w-full flex flex-col items-center text-center px-1">
                  ...
                  <div
                    className="font-bold text-sm leading-tight text-white mb-3 line-clamp-1 w-full group-hover:text-yellow-100 transition-colors"
                  ...
                  {item.amount > 1 ? (
                    <div className="w-full flex flex-col gap-1.5">
                      ...
                      {item.amount > 2 && (
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            sellAllDuplicates(item.card.id, item.amount);
                          }}
                        ...
                        >
                          Залишити одну (+{(item.amount - 1) * currentSellPrice}{" "}
                          <Coins size={10} className="inline text-yellow-500" />
                          )
                        </button>
                      )}
                    ...
                    <div className="w-full flex justify-center items-center text-xs py-2.5 text-neutral-600 bg-neutral-950/50 rounded-lg border border-neutral-800 font-medium">
                      Один екземпляр ({currentSellPrice}{" "}
                      <Coins size={12} className="inline ml-1 opacity-50" />)
                    </div>
                  )}
                </div>
              </div>
  ...
  if (loading)
    return (
      <div className="text-center py-20 text-neutral-500">
        <Loader2 className="animate-spin mx-auto mb-4 w-12 h-12" /> Завантаження
        Залу Слави...
      </div>
    );
    ...
    <div className="max-w-3xl mx-auto pb-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="bg-neutral-900 border border-neutral-800 rounded-3xl overflow-hidden shadow-xl">
        {filteredLeaders.map((leader, index) => {
          const realRank =
            allProfiles.findIndex((p) => p.uid === leader.uid) + 1;

          return (
            <div
              key={leader.uid}
              onClick={() => setViewingPlayerProfile(leader.uid)}
              className={`flex items-center justify-between p-4 border-b border-neutral-800/50 last:border-0 transition-colors cursor-pointer group ${
                leader.uid === currentUid
                  ? "bg-yellow-900/10"
                  : "hover:bg-neutral-800/80"
              } ${leader.isBanned ? "opacity-50" : ""}`}
            >
              <div className="flex items-center gap-4">
                <div
                  className={`w-10 h-10 flex items-center justify-center font-black text-lg rounded-xl border transition-transform group-hover:scale-110 ${
                    realRank === 1
                ...
                <div>
                  <div className="font-bold text-white flex items-center gap-2 text-lg">
                    ...
                    {leader.uid === currentUid && (
                      <span className="text-[10px] bg-yellow-500/20 text-yellow-500 px-2 py-0.5 rounded-full ml-2">
                        ВИ
                      </span>
                    )}
                  </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <ArrowLeft
                  size={16}
                  className="text-neutral-600 group-hover:text-yellow-500 transform rotate-180 transition-colors"
                />
              </div>
            </div>
          );
...
function PublicProfileView({
      ...
      try {
        ...
        if (foundProfile) {
          const invSnap = await getDocs(
            collection(db, "artifacts", appId, "users", targetUid, "inventory")
          );
          const invList = [];
          invSnap.forEach((doc) => invList.push({ id: doc.id, ...doc.data() }));

          const fullInv = invList
            .map((item) => {
              const cardData = cardsCatalog.find((c) => c.id === item.id);
              return cardData ? { card: cardData, amount: item.amount } : null;
            })
  ...
  if (loading)
    return (
      <div className="text-center py-20 text-neutral-500">
        <Loader2 className="animate-spin mx-auto w-12 h-12 mb-4" /> Завантажуємо
        дані гравця...
      </div>
    );
  if (!playerInfo)
    return (
      <div className="text-center py-20 text-red-500">
        Гравець не знайдений або його акаунт видалено.{" "}
        <button onClick={goBack} className="block mx-auto mt-4 underline">
          Повернутись
        </button>
      </div>
    );
  ...
  return (
    <div className="animate-in slide-in-from-right-8 duration-500">
      ...
      <div className="bg-neutral-900 border border-neutral-800 rounded-3xl p-8 text-center relative overflow-hidden mb-8">
        ...
        ></div>
        <div
          className={`w-24 h-24 bg-neutral-800 rounded-full flex items-center justify-center font-black text-4xl border-4 shadow-xl mx-auto relative z-10 mb-4 ${
            playerInfo.isBanned
              ...
              : "text-blue-500 border-neutral-700"
          }`}
        >
          {playerInfo.isBanned ? (
            <Ban size={48} />
        ...
        <h2 className="text-3xl font-black text-white mb-1 relative z-10 flex justify-center items-center gap-2">
          {playerInfo.nickname}
          {playerInfo.isBanned && (
            <span className="text-[10px] bg-red-900/50 text-red-400 px-2 py-1 rounded-full uppercase tracking-widest border border-red-800">
              Бан
            </span>
          )}
        </h2>
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-4">
          {playerInventory.map((item, index) => {
            ...
            return (
              ...
              >
                <div
                  className={`relative w-full aspect-[2/3] rounded-xl border-2 overflow-hidden bg-neutral-900 mb-2 ${style.border}`}
                >
                  {item.amount > 1 && (
                ...
                <div className="w-full text-center px-1">
                  <div
                    className={`text-[8px] font-black uppercase tracking-widest mb-0.5 ${style.text}`}
                  >
                    {item.card.rarity}
                  </div>
                  <div className="font-bold text-[10px] sm:text-xs leading-tight text-white truncate w-full">
                    {item.card.name}
                  </div>
                </div>
              </div>
...
function ProfileView({
  ...
  const handlePromoSubmit = async (e) => {
    ...
    // Старі системні коди для отримання адмінки
    ...
    // Логіка користувацьких промокодів з бази
    try {
      const promoRef = doc(
        db,
        "artifacts",
      ...
      if (
        pData.maxGlobalUses > 0 &&
        pData.currentGlobalUses >= pData.maxGlobalUses
      ) {
      ...
      if (pData.maxUserUses > 0 && uUses >= pData.maxUserUses) {
        showToast(
          "Ви вже використали цей промокод максимально можливу кількість разів.",
          "error"
        );
        return;
      }
      ...
      await batch.commit();
      showToast(
        `Промокод застосовано! Отримано ${pData.reward} монет.`,
        "success"
      );
      setPromoInput("");
    } catch (err) {
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="bg-neutral-900 border border-neutral-800 rounded-3xl p-8 text-center relative overflow-hidden">
        ...
        <div
          className={`w-24 h-24 bg-neutral-800 rounded-full flex items-center justify-center font-black text-4xl border-4 shadow-xl mx-auto relative z-10 mb-4 ${
            profile?.isSuperAdmin
              ...
              : profile?.isAdmin
              ? "text-purple-400 border-purple-500"
              : "text-yellow-500 border-neutral-700"
          }`}
        >
          {profile?.isSuperAdmin ? (
            <Crown size={48} />
        ...
        <div className="grid grid-cols-2 gap-4 relative z-10 mt-4">
          <div className="bg-neutral-950 border border-neutral-800 rounded-2xl p-4 flex flex-col items-center">
            ...
            <span className="text-xs text-neutral-500 font-bold uppercase mt-1">
              Монети
            </span>
          </div>
          <div className="bg-neutral-950 border border-neutral-800 rounded-2xl p-4 flex flex-col items-center">
      ...
      <div className="bg-neutral-900 border border-neutral-800 rounded-3xl p-6">
        <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
          <Gift className="text-purple-500" /> Промокод
        </h3>
        <form onSubmit={handlePromoSubmit} className="flex gap-3">
          <input
            ...
            className="flex-1 bg-neutral-950 border border-neutral-700 rounded-xl px-4 py-3 text-white uppercase"
          />
          <button
            type="submit"
            className="bg-purple-600 text-white font-bold py-3 px-6 rounded-xl"
          >
            Застосувати
          </button>
        </form>
      ...
      <button
        onClick={handleLogout}
        className="w-full bg-neutral-900 border border-red-900/50 hover:bg-red-900/20 p-4 rounded-xl text-left transition-colors flex justify-between group"
      >
...
function AdminView({
  ...
  // Для перегляду інвентарю гравця
  ...
  useEffect(() => {
    ...
    if (activeTab === "promos" && currentProfile.isSuperAdmin) {
      const unsub = onSnapshot(
        collection(db, "artifacts", appId, "public", "data", "promoCodes"),
        (snap) => {
          const pList = [];
  ...
  const handleDeleteUser = async (userToDelete) => {
    ...
    if (userToDelete.isAdmin && !currentProfile.isSuperAdmin)
      return showToast(
        "У вас немає прав видаляти інших адміністраторів!",
        "error"
      );
    if (
      !confirm(
  ...
  const handleInspectUser = async (uid) => {
    ...
    try {
      ...
      console.error(e);
      showToast("Помилка доступу до інвентарю користувача.");
    }
  ...
  const submitBan = async (e) => {
    ...
    let banUntil = null;
    if (banDuration === "1d")
      banUntil = new Date(Date.now() + 86400000).toISOString();
    if (banDuration === "7d")
      banUntil = new Date(Date.now() + 7 * 86400000).toISOString();
  ...
  const handleUnban = async (uid) => {
    try {
      await updateDoc(
        doc(db, "artifacts", GAME_ID, "public", "data", "profiles", uid),
        {
          isBanned: false,
          banReason: null,
          banUntil: null,
  ...
  // --- Функції нарахування собі та іншим ---
  ...
  const giveCardToUser = async () => {
    ...
    try {
      ...
      batch.set(
        invRef,
        { amount: increment(adminAddCardAmount) },
        { merge: true }
      );
  ...
  const giveCoinsToUser = async () => {
    ...
    try {
      ...
      showToast(
        `Успішно ${actionText} ${Math.abs(adminAddCoinsAmount)} монет.`,
        "success"
      );

      setViewingUser((prev) => ({
        ...prev,
        coins: prev.coins + adminAddCoinsAmount,
      }));
  ...
  // --- Функції збереження та видалення Паків/Карт ---
  const savePack = async (e) => {
    ...
    if (editingPack) {
      updatedPacks = updatedPacks.map((p) =>
        p.id === editingPack.id
          ? { ...packForm, id: editingPack.id, cost: Number(packForm.cost) }
          : p
      );
    } else {
      updatedPacks.push({
    ...
    await updateDoc(
      doc(db, "artifacts", appId, "public", "data", "gameSettings", "main"),
      { packs: updatedPacks }
    );
    setEditingPack(null);
  ...
  const saveCard = async (e) => {
    ...
    if (editingCard) {
      updatedCatalog = updatedCatalog.map((c) =>
        c.id === editingCard.id ? newCardData : c
      );
    } else {
  ...
  const savePromo = async (e) => {
    ...
    try {
      await setDoc(
        doc(db, "artifacts", appId, "public", "data", "promoCodes", codeId),
        {
          code: codeId,
    ...
    <div className="max-w-4xl mx-auto pb-10 relative">
      ...
      <div className="flex gap-2 mb-6 bg-neutral-900 p-2 rounded-xl overflow-x-auto">
        <button
          ...
          className={`flex-1 min-w-[100px] py-3 rounded-lg font-bold flex justify-center items-center gap-2 ${
            activeTab === "users"
      ...
      {activeTab === "users" && (
        <div className="bg-neutral-900 rounded-2xl border border-neutral-800 p-4 relative">
          {/* Блок швидкого нарахування собі */}
          {!viewingUser && (
            <div className="bg-neutral-950 p-4 rounded-xl border border-neutral-800 mb-6 flex flex-col sm:flex-row gap-3 items-end sm:items-center">
              <span className="text-white font-bold flex-1 w-full flex items-center gap-2">
          ...
          {viewingUser ? (
            <div className="animate-in fade-in slide-in-from-right-4">
              ...
              <div className="bg-neutral-950 p-4 rounded-xl border border-neutral-800 mb-4 flex flex-col sm:flex-row gap-3 items-end sm:items-center">
                ...
                <div className="w-full sm:w-24">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setAdminAddCardAmount(Number(e.target.value))
                    }
                    className="w-full bg-neutral-900 border border-neutral-700 rounded-lg px-3 py-2 text-white outline-none focus:border-purple-500"
                  />
                </div>
              ...
              <div className="bg-neutral-950 p-4 rounded-xl border border-neutral-800 mb-6 flex flex-col sm:flex-row gap-3 items-end sm:items-center">
                <div className="flex-1 w-full">
                  <label className="text-xs text-neutral-400 font-bold mb-1 block">
                    Нарахувати / Відняти монети гравцю (можна з мінусом):
                  </label>
                  <input
                    type="number"
                    value={adminAddCoinsAmount}
                    onChange={(e) =>
                      setAdminAddCoinsAmount(Number(e.target.value))
                    }
                ...
                <button
                  ...
                  className="bg-yellow-600 hover:bg-yellow-500 text-yellow-950 font-bold px-4 py-2 rounded-lg w-full sm:w-auto transition-colors h-10"
                >
                  Застосувати
                </button>
              </div>
              ...
              <h3 className="text-xl font-bold text-white mb-2 flex items-center gap-2">
                Інвентар:{" "}
                <span className="text-yellow-500">{viewingUser.nickname}</span>{" "}
                ...
                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3 mt-6">
                  {userInventory.map((invItem) => {
                    ...
                    return (
                      <div
                        key={invItem.id}
                        className={`bg-neutral-950 rounded-xl border-2 ${style.border} overflow-hidden flex flex-col items-center p-1 relative group`}
                      >
                        <img
                          src={c.image}
                          alt={c.name}
                        ...
                        <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                          <button
                            onClick={() =>
                              removeCardFromUser(invItem.id, invItem.amount)
                            }
                  ...
                  >
                    <div>
                      <div className="font-bold text-white flex items-center gap-2">
                        ...
                        {u.isBanned && (
                          <span className="text-[10px] bg-red-900/50 text-red-400 px-2 py-0.5 rounded border border-red-800 uppercase font-black tracking-widest">
                            Бан
                          </span>
                        )}
                      </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      <div className="hidden sm:block text-right mr-2">
                        ...
                        <div className="text-sm font-bold text-yellow-500">
                          {u.coins}{" "}
                          <Coins size={12} className="inline text-yellow-600" />{" "}
                          /{" "}
                        ...
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-lg text-[10px] font-bold transition-colors border ${
                            u.isAdmin
                              ...
                              : "bg-purple-900/40 text-purple-400 border-purple-800 hover:bg-purple-900/60"
                          }`}
                        >
                      ...
                      {canBan &&
                        (u.isBanned ? (
                          ...
                          <button
                            ...
                            className="p-2 bg-red-900/40 text-red-400 hover:bg-red-900 rounded-lg transition-colors"
                            title="Заблокувати"
                          >
                            <Ban size={18} />
                          </button>
      ...
      {/* --- Вкладка: ПРОМОКОДИ (ТІЛЬКИ ДЛЯ СУПЕР АДМІНА) --- */}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setPromoForm({ ...promoForm, reward: e.target.value })
                  }
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setPromoForm({ ...promoForm, maxUserUses: e.target.value })
                  }
                  className="w-full bg-neutral-950 border border-neutral-700 rounded-xl px-4 py-3 text-white"
                  min="0"
                  required
                  ...
                  <div className="text-right text-xs text-neutral-400">
                    <div>
                      ...
                      <span className="text-white">
                        {p.currentGlobalUses} /{" "}
                        {p.maxGlobalUses === 0 ? "∞" : p.maxGlobalUses}
                      </span>
                    </div>
                    <div>
                      На 1 гравця:{" "}
        ...
        <div className="space-y-6">
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
              ...
              <input
                ...
                value={packForm.cost}
                onChange={(e) =>
                  setPackForm({ ...packForm, cost: e.target.value })
                }
                className="bg-neutral-950 border border-neutral-700 rounded-xl px-4 py-3 text-white"
                min="0"
                required
              ...
              <input
                ...
                onChange={(e) =>
                  setPackForm({ ...packForm, image: e.target.value })
                }
                className="bg-neutral-950 border border-neutral-700 rounded-xl px-4 py-3 text-white sm:col-span-2"
                required
              />
              ...
              <div className="sm:col-span-2 mt-2 p-4 border border-neutral-800 rounded-xl bg-neutral-950/50">
                <h4 className="text-neutral-400 text-sm font-bold mb-3">
                  Кастомні шанси випадіння (залиште пустим, щоб використовувати
                  глобальні):
                </h4>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {filteredPacks.map((pack) => (
              <div
                key={pack.id}
                className="bg-neutral-900 rounded-xl p-4 border border-neutral-800 relative group"
              >
                <img
                  src={pack.image}
                  alt={pack.name}
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-4">
              ...
              <select
                ...
                onChange={(e) =>
                  setCardForm({ ...cardForm, packId: e.target.value })
                }
                className="bg-neutral-950 border border-neutral-700 rounded-xl px-4 py-3 text-white"
                required
              >
              ...
              <select
                ...
                onChange={(e) =>
                  setCardForm({ ...cardForm, rarity: e.target.value })
                }
                className="bg-neutral-950 border border-neutral-700 rounded-xl px-4 py-3 text-white"
              >
                {rarities.map((r) => (
              ...
              <input
                ...
                onChange={(e) =>
                  setCardForm({ ...cardForm, maxSupply: e.target.value })
                }
                className="bg-neutral-950 border border-neutral-700 rounded-xl px-4 py-3 text-white"
              />
              ...
              <input
                ...
                onChange={(e) =>
                  setCardForm({ ...cardForm, weight: e.target.value })
                }
                className="bg-neutral-950 border border-neutral-700 rounded-xl px-4 py-3 text-white"
              ...
              <input
                ...
                onChange={(e) =>
                  setCardForm({ ...cardForm, sellPrice: e.target.value })
                }
              ...
              <input
                ...
                onChange={(e) =>
                  setCardForm({ ...cardForm, image: e.target.value })
                }
                className="bg-neutral-950 border border-neutral-700 rounded-xl px-4 py-3 text-white md:col-span-2"
                required
              />
            </div>
                ...
                >
                  ...
                  <div className="p-2 text-center flex flex-col items-center flex-1 justify-between">
                    ...
                    <div className="w-full flex justify-center gap-1 mt-1">
                      {card.weight && (
                        <div
                          className="text-[9px] text-yellow-500/80 font-bold bg-yellow-500/10 px-1.5 py-0.5 rounded border border-yellow-500/20"
                          title="Індивідуальна вага"
                        >
                          ⚖️ {card.weight}
                        </div>
                      )}
                      <div
                        className="text-[9px] text-green-400 font-bold bg-green-500/10 px-1.5 py-0.5 rounded border border-green-500/20"
                        title="Ціна продажу дубліката"
                      >
                        +{card.sellPrice || SELL_PRICE}{" "}
                        <Coins size={8} className="inline" />
                      </div>
                    </div>
                  </div>
                </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className={`w-full aspect-[2/3] rounded-3xl border-4 overflow-hidden shadow-[0_20px_70px_rgba(0,0,0,0.8)] ${style.border} relative group`}
        >
          <img
            src={card.image}
            alt={card.name}
        ...
        <div className="mt-8 flex flex-col items-center text-center w-full">
          <h3 className="font-black text-4xl text-white mb-6 drop-shadow-xl">
            {card.name}
          </h3>
        </div>
      </div>
